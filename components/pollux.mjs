 ...
 * Pollux
 *
 * more info on https://pollux.gouz.dev
 *
 * This component displays the results page and a qrcode for your poll
 *
 * `!pollux(choice1|choice2|...)`
 *
...
export default async (data) => {
  let newData = data;
  const occurences = [...newData.matchAll(/!pollux\((.*)\)/g)];
  for await (const match of occurences) {
    const [choices, ...options] = match[1].split(",").map((a) => a.trim());
    const [choix, name] = choices.split("::").map((a) => a.trim());
    const uuid = Bun.randomUUIDv7();
    const hash = LZString.compressToEncodedURIComponent(`${uuid}|${choix}`);
    newData = newData.replace(
      match[0],
      `<div class="pollux">
  <iframe src="https://pollux.gouz.dev/results#${hash}" width="600" height="600" frameBorder="0"></iframe>
  <div class="sd-qrcode" data-url="https://pollux.gouz.dev/${(options.shift() ?? "false") === "true" ? "many" : "vote"}#${hash}" style="width: 400px"></div>
  <script>window.slidesk_pollux_${name} = "${uuid}";</script>
</div>`,
    );
  }
  return newData;
